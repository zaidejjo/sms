use sms_core::{Expr, evaluate, evaluate_complex, Parser as CoreParser, EquationSolver, SeriesOp, compute_series, AISolver, Fraction, ExportData};
...
#[command(name = "sms", version, about = "Smart Math Solver")]
struct Args {
    /// Launch the TUI (Terminal User Interface) instead of CLI
...
fn main() -> Result<()> {
    ...
    loop {
        ...
        if input.starts_with("ai ") {
            ...
            let mut parser = CoreParser::new(rest);
        ...
        let mut parser = CoreParser::new(input);
        let expr = parser.parse_equation();
...
fn parse_series(input: &str, op: SeriesOp) -> Option<f64> {
    ...
    let mut parser = CoreParser::new(expr_str);
